...
casper.eachThen(prefCodes, function reqAndGrab() {
    casper.echo('[' +i+ '] of ['+ prefCodes.length +'] '+ prefCodes[i]);
...
*/  
    casper.then(function grabData(){
        var currentPrefCode = prefCodes[i];
        casper.echo("Current Pref is" + currentPrefCode + "prefcodes[i] is now " + prefCodes[i]+ " i is now "+ i);
        casper.evaluate(function(areaCode){
          document.querySelector('#sStoreKeyword').value = areaCode; 
        ...
        for(var j = 0; j< names.length; j++) { 
            shopsOnThisPage[j]= [parseFloat(latLongs[j][0]),parseFloat(latLongs[j][1]), names[j]];
        }
            

        return shopsOnThisPage;      
        });
...
casper.then(function(){
    ...
    onlyUnique(shopInfo);
    onlyUnique(shopInfo);
    saveToFile(shopInfo, "currys-uk");
    saveToLog(shopInfo, "currys-uk");
