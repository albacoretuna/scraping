...
*/  
    casper.then(function grabData(){
        ...
        for(var j = 0; j< names.length; j++) { 
            shopsOnThisPage[j]= [parseFloat(latLongs[j][1]),parseFloat(latLongs[j][1]), names[j]];
        }
...
casper.then(function(){
    casper.echo("shopinfo is now: " +JSON.stringify(shopInfo)+ " it has lenght of "+ shopInfo.length );
    casper.echo();
    });
casper.run();
