...
for(var x = 0; x < codeAreasTotal; x= x+5){
    prefCodesOneFifth.push(originalArray[x]);
    }
    return prefCodesOneFifth;
}
prefixes = prefCodesMinimizer(prefixes);
...
casper.eachThen(prefixes, function(){
casper.then(function fillTheForm(){
    ...
    casper.waitForSelector('#searchLocationMain' ,function isFormReady() {    
        casper.fillSelectors('form#searchLocationMain',
                 { '#search_store_location' : prefixes[i]+', uk'}, true);
    });
